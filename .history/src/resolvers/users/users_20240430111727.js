 ...
 Mutation: {
  login: async (parent, args, { dataSources }, context) => {
    const getData = new Promise((resolve) => {
      dataSources.mongoAPI.getItemData({ db: 'tasksDB', collection: 'users', query: { "email": args.payload.email, "password": args.payload.password }, })
        .then((data, err) => resolve(data))
    }).then((data) => {
      return data
    })
    return getMember.then(data => data).catch((err) => console.log(err))
  },

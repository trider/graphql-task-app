  ...
  updateTask: async (parent, args, { dataSources }, context) => {
   const getData = new Promise((resolve) => {
    dataSources.mongoAPI.updateItemData(
      { db: 'tasksDB', collection: 'tasks', query: { 
        "_id": ObjectID(args.payload.id) }, data: { $set: { ...args.payload, updated: new Date() } } }
    ).then((data, err) => resolve(data))  
  }).then((data) => {
    console.log(data)
    return dataSources.mongoAPI.getItemData({ db: 'tasksDB', collection: 'tasks', query: { "_id": ObjectID(args.payload.id) } })
  })
  return getData.then(data => data).catch((err) => {
    console.log(err)
  })
